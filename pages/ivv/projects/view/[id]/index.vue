...
const fetchProjectData = async () => {
  try {
    ...
    if (response.statusCode === 200) {
      ...
      tableData.value = response.data.documentTypes;
    } else {
...
<template>
  <div>
      ...
      <rs-table :data="tableData" :loading="isLoading" advanced>
        <template v-slot:tindakan="data">
          <div class="flex gap-2">
            <NuxtLink :to="`/ivv/projects/document/${data.value.id}?projectId=${route.params.id}`">
      ...
      <template #body>
        <FormKit type="form" :actions="false" @submit="handleAddSubmit">
          ...
          <div class="flex justify-end gap-2">
            <rs-button variant="primary" btn-type="submit"> Tambah </rs-button>
          </div>
        </FormKit>
      </template>
    ...
    >
      ...
      <template #footer>
        <rs-button variant="danger" @click="confirmDelete"> Padam </rs-button>
        <rs-button @click="isDeleteModalVisible = false"> Batal </rs-button>
      </template>
    </RsModal>
  </div>
</template>
